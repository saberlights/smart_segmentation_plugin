...
from plugins.saberlights_smart_segmentation_plugin import plugin as plugin_module
from plugins.saberlights_smart_segmentation_plugin.plugin import (
...
def _reset_global_state() -> None:
    ...
    plugin_module._prepared_segment_queue_by_stream.clear()
    ...
    plugin_module._follow_up_prompt_batches_by_stream.clear()
...
def test_get_components_registers_hook_handlers_for_current_host() -> None:
    ...
    assert "smart_segmentation_planner_follow_up_barrier" in hook_components
    assert (
        hook_components["smart_segmentation_planner_follow_up_barrier"]["metadata"]["hook"]
...
def test_prepared_segments_consume_next_reply_when_host_typo_postprocess_changes_text() -> None:
    """宿主错别字后处理会改变 after_build 文本，缓存不能只靠文本 hash。"""
    _reset_global_state()
    try:
        ok = _store_prepared_segments(
            "stream-typo",
            "都要死了还搁这嘴硬呢\n\n赶紧打车回去别硬扛，回家就躺着，别真给我撂挑子\n\n等你养好了再继续算总账",
            [
                "都要死了还搁这嘴硬呢",
                "赶紧打车回去别硬扛，回家就躺着，别真给我撂挑子",
                "等你养好了再继续算总账",
        ...
        # 模拟宿主 chinese_typo 在 replyer 与 after_build 之间把“撂”改成“料”，
        # 旧实现会因为 hash 漂移而 miss，随后整段原文被 send_service 发出去。
        segments = _pop_prepared_segments(
            "stream-typo",
            "都要死了还搁这嘴硬呢 赶紧打车回去别硬扛，回家就躺着，别真给我料挑子 等你养好了再继续算总账",
        )
        ...
        assert not plugin_module._prepared_segment_registry
        assert not plugin_module._prepared_segment_queue_by_stream
    finally:
        _reset_global_state()
...
def test_planner_before_request_waits_and_injects_missing_follow_up_segments() -> None:
    """内部连续 planner 不经过 chat.receive.before_process，必须在 planner hook 挡住。

    宿主会先构建 prompt 再调用 maisaka.planner.before_request；所以 hook 等到补发完成后，
    如果当前 messages 仍缺少后续分段，就必须返回 modified_kwargs.messages 注入 bot 自身
    已发送的补发段。
    """
    _reset_global_state()
    plugin = SmartSegmentationPlugin()
    plugin_module._register_follow_up_prompt_batch("stream-planner", ["第二段", "第三段"])
    ...
    async def _run() -> dict[str, Any]:
        follow_up_task = asyncio.create_task(_blocked_send_segments())
        plugin_module._track_follow_up_task(follow_up_task, stream_id="stream-planner")
        ...
        planner_task = asyncio.create_task(
            plugin.handle_maisaka_planner_before_request(
                ...
                messages=[
                    ...
                    {"role": "user", "content": '<message user="MaiBot" is_self_message="true">\n第一段'},
        ...
        blocked = False
        try:
            await asyncio.wait_for(asyncio.shield(planner_task), timeout=0.05)
        except asyncio.TimeoutError:
            blocked = True
        assert blocked is True

        release_send.set()
        return await asyncio.wait_for(planner_task, timeout=0.2)

    try:
        result = asyncio.run(_run())
        assert result["action"] == "continue"
        updated_messages = result["modified_kwargs"]["messages"]
        assert updated_messages[2] == {
            "role": "assistant",
            "content": '<message user="MaiBot" is_self_message="true">\n第二段\n第三段',
        }
        assert plugin_module._get_unconsumed_follow_up_prompt_segments("stream-planner") == []
    finally:
        _reset_global_state()


def test_planner_before_request_does_not_inject_when_follow_up_segments_already_visible() -> None:
    _reset_global_state()
    plugin = SmartSegmentationPlugin()
    plugin_module._register_follow_up_prompt_batch("stream-visible", ["第二段"])
    ...
    try:
        ...
        assert result == {"action": "continue"}
        assert plugin_module._get_unconsumed_follow_up_prompt_segments("stream-visible") == []
    finally:
        _reset_global_state()
